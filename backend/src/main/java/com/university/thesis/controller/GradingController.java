...
public class GradingController {
    ...
    public ResponseEntity<ScoreSummaryResponse> getSummary(
            @PathVariable Long thesisId,
            @AuthenticationPrincipal User currentUser) {
        return ResponseEntity.ok(gradingService.getScoreSummary(thesisId, currentUser));
    }
    ...
    /**
     * POST /api/scores/draft
     */
    @PostMapping("/draft")
    public ResponseEntity<GradeResponse> saveDraft(
            @AuthenticationPrincipal User currentUser,
            @Valid @RequestBody GradeRequest request) {
        return ResponseEntity.ok(gradingService.saveDraft(currentUser.getId(), request));
    }
    ...
    public ResponseEntity<GradeResponse> submitGrade(
            @AuthenticationPrincipal User currentUser,
            @Valid @RequestBody GradeRequest request) {
        return ResponseEntity.ok(gradingService.submitGrade(currentUser.getId(), request));
    }
}
