...
import { Users, BookOpen, ShieldCheck, GraduationCap, School, UserCog, TrendingUp, CheckCircle, AlertTriangle } from 'lucide-react';
import {
...
function UserManagement() {
  ...
  const emptyForm = { name: '', email: '', phone: '', role: 'SINHVIEN', password: '', status: 'ACTIVE' };
  ...
  return (
    <>
      <div className="grid-4 mb-20">
        {stats.map(s => {
          const cfg = {
            ADMIN:     { Icon: ShieldCheck, color: '#DC2626', bg: '#FEF2F2' },
            GIAOVU:    { Icon: School,      color: '#7C3AED', bg: '#EDE9FE' },
            GIANGVIEN: { Icon: UserCog,     color: '#0072CC', bg: '#DBEAFE' },
            SINHVIEN:  { Icon: GraduationCap, color: '#059669', bg: '#D1FAE5' },
          }[s.role] || { Icon: Users, color: '#0072CC', bg: '#DBEAFE' };
          return (
            <div key={s.role} className="card" style={{ marginBottom: 0 }}>
              <div className="card-body" style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                <div style={{ width: 46, height: 46, borderRadius: 12, background: cfg.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
      ...
      >
        ...
        <div className="grid-2">
          <div className="form-group">
            ...
            {editing && <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 4 }}>Email không thể thay đổi sau khi tạo</div>}
          </div>
          <div className="form-group">
            <label className="form-label">Số điện thoại</label>
            <input className="form-input" type="tel" placeholder="09xx..." value={form.phone} onChange={e => setForm({ ...form, phone: e.target.value })} />
          </div>
        </div>
      ...
      >
        <div style={{ textAlign: 'center', padding: '8px 0 16px' }}>
          <AlertTriangle size={48} color="var(--danger)" style={{ margin: '0 auto 12px' }} />
          <p>Bạn có chắc muốn xóa tài khoản <strong>{deleteTarget?.name}</strong>?</p>
        </div>
      </Modal>
    </>
  );
...
function AdminStatistics() {
  ...
  return (
    <div>
      <div className="grid-4 mb-20">
        {[
          { label: 'Tổng khoá luận',  value: stats.overview.totalThesis ?? '-',   Icon: BookOpen,     color: '#0072CC', bg: '#DBEAFE' },
          { label: 'Hội đồng đã tạo', value: stats.overview.totalCouncils ?? '-', Icon: School,       color: '#7C3AED', bg: '#EDE9FE' },
          { label: 'Đã hoàn thành',   value: stats.overview.completed ?? '-',     Icon: CheckCircle,  color: '#059669', bg: '#D1FAE5' },
          { label: 'Điểm TB chung',   value: stats.overview.avgScore ? stats.overview.avgScore.toFixed(1) : '-', Icon: TrendingUp, color: '#D97706', bg: '#FEF3C7' },
        ].map(s => (
...
function AdminOverview() {
  ...
  return (
    <div>
      <div className="grid-4 mb-20">
        {[
          { label: 'Tổng người dùng', value: counts.total,     Icon: Users,          color: '#003D7A', bg: '#DBEAFE' },
          { label: 'Giáo vụ',         value: counts.giaovu,    Icon: School,         color: '#7C3AED', bg: '#EDE9FE' },
          { label: 'Giảng viên',      value: counts.giangvien, Icon: UserCog,        color: '#0072CC', bg: '#E0F2FE' },
          { label: 'Sinh viên',       value: counts.sinhvien,  Icon: GraduationCap,  color: '#059669', bg: '#D1FAE5' },
        ].map(c => (
          <div key={c.label} className="card" style={{ marginBottom: 0, cursor: 'pointer' }} onClick={() => navigate('/admin/users')}>
            <div className="card-body" style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
...
function AdminDashboard() {
  ...
  return (
    <MainLayout pageTitle={activeTab?.label || 'Admin'} breadcrumb={`Admin / ${activeTab?.label || ''}`}>
