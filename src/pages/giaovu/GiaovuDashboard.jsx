      ...
      >
        ...
        <div className="form-group">
          ...
          <div style={{ maxHeight: 160, overflowY: 'auto', border: '1.5px solid var(--border)', borderRadius: 8, padding: 8 }}>
            ...
            {students.length === 0 && <div style={{ color: 'var(--text-muted)', fontSize: 13, padding: 8 }}>Không có sinh viên</div>}
          </div>
        </div>
      ...
      >
        <div className="form-group">
          ...
          <select
            ...
            onChange={e => setReviewerForm({ lecturerId: e.target.value })}
          >
        ...
        <div style={{ background: '#FFF7E6', borderRadius: 8, padding: '10px 14px', fontSize: 13, color: '#92400E' }}>
          ⚠️ Hệ thống sẽ tự động gửi email thông báo đến giảng viên được phân công.
        </div>
      </Modal>
    </>
  );
...
function CouncilTab() {
  ...
  const emptyForm = {
    name: '', date: '', room: '',
    members: [], // { lecturerId, councilRole }
    thesisIds: [],
  };
  ...
  return (
    <>
                  ...
                  <tr key={c.id}>
                    ...
                    <td>
                      <div style={{ display: 'flex', gap: 6 }}>
                        ...
                        <button className="btn btn-success btn-sm" onClick={() => handleExportPDF(c)}>📄 PDF</button>
                      </div>
                    </td>
                  </tr>
                ))}
        ...
        {selectedCouncil && (
          <div>
            <div className="grid-2 mb-16">
              ...
              <div><strong>Trạng thái:</strong> {selectedCouncil.status === 'LOCKED' ? '🔒 Đã khoá' : '🟢 Đang hoạt động'}</div>
      ...
      <Modal
        isOpen={showCreate}
        onClose={() => setShowCreate(false)}
        title="Thành lập Hội đồng Bảo vệ"
        size="xl"
        footer={
          <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
            <button className="btn btn-outline" onClick={() => setShowCreate(false)}>Huỷ</button>
            <button className="btn btn-primary" onClick={handleCreate}>Thành lập hội đồng</button>
          </div>
        }
      >
        <div className="grid-2 mb-16">
        ...
        <div className="grid-2">
                ...
                {form.members.map(m => {
                  ...
                  return (
                    <div key={m.lecturerId} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 0', borderBottom: '1px solid var(--border)' }}>
                      <span style={{ flex: 1, fontSize: 13 }}>{lec?.name}</span>
                      <select
                        className="form-select"
          ...
          <div className="form-group">
            ...
            <div style={{ maxHeight: 240, overflowY: 'auto', border: '1.5px solid var(--border)', borderRadius: 8, padding: 8 }}>
              ...
              {theses.length === 0 && <div style={{ color: 'var(--text-muted)', fontSize: 13, padding: 8 }}>Không có khoá luận</div>}
            </div>
    ...
    <div>
      <div className="grid-4 mb-20">
        {[
          ...
          { label: 'Đang thực hiện', value: stats.overview.inProgress ?? '-', icon: '⏳' },
...
function GiaovuOverview() {
  ...
  const cards = [
    { label: 'Tổng khoá luận', value: counts.thesis, icon: '📝', color: '#005BAA', path: '/giaovu/thesis' },
    { label: 'Hội đồng', value: counts.councils, icon: '🏛️', color: '#10B981', path: '/giaovu/councils' },
    { label: 'Tiêu chí chấm', value: counts.criteria, icon: '⭐', color: '#F59E0B', path: '/giaovu/criteria' },
  ];
...
function GiaovuDashboard() {
  ...
  const tabs = [
    ...
    { path: '/giaovu/councils', label: '🏛️ Hội đồng' },
  ...
  return (
    <MainLayout pageTitle={activeTab?.label?.replace(/^[\S]+ /, '') || 'Giáo vụ'} breadcrumb={`Giáo vụ / ${activeTab?.label?.replace(/^[\S]+ /, '') || ''}`}>
